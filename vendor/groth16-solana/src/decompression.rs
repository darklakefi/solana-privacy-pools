...
use pinocchio::syscalls::sol_alt_bn128_compression;

// Compression operation constants
const G1_DECOMPRESS: u64 = 0;
const G2_DECOMPRESS: u64 = 1;

fn alt_bn128_g1_decompress(compressed: &[u8; 32]) -> Result<[u8; 64], u64> {
    let mut result = [0u8; 64];
    let ret = unsafe {
        sol_alt_bn128_compression(G1_DECOMPRESS, compressed.as_ptr(), 32, result.as_mut_ptr())
    };
    if ret == 0 {
        Ok(result)
    } else {
...
fn alt_bn128_g2_decompress(compressed: &[u8; 64]) -> Result<[u8; 128], u64> {
    let mut result = [0u8; 128];
    let ret = unsafe {
        sol_alt_bn128_compression(G2_DECOMPRESS, compressed.as_ptr(), 64, result.as_mut_ptr())
    };
    if ret == 0 {
        Ok(result)
    } else {
...
mod tests {
    ...
    fn convert_endianness(bytes: &[u8]) -> Vec<u8> {
