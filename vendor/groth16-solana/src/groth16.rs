...
fn alt_bn128_addition(input: &[u8]) -> Result<Vec<u8>, u64> {
    ...
    let ret = unsafe {
        sol_alt_bn128_group_op(ALT_BN128_ADD, input.as_ptr(), input.len() as u64, result.as_mut_ptr())
    };
    if ret == 0 {
        Ok(result)
    } else {
...
fn alt_bn128_multiplication(input: &[u8]) -> Result<Vec<u8>, u64> {
    let mut result = vec![0u8; 64];
    let ret = unsafe {
        sol_alt_bn128_group_op(ALT_BN128_MUL, input.as_ptr(), input.len() as u64, result.as_mut_ptr())
    };
    if ret == 0 {
        Ok(result)
    } else {
...
fn alt_bn128_pairing(input: &[u8]) -> Result<Vec<u8>, u64> {
