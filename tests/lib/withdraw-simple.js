...
async function withdrawSimple(
    ...
    const depositIndex = allDeposits.findIndex(d => {
        ...
        const dCommit = Buffer.isBuffer(d.commitment) ? d.commitment : Buffer.from(d.commitment);
        const infoCommit = Buffer.isBuffer(depositInfo.commitment) ? depositInfo.commitment : Buffer.from(depositInfo.commitment);
    ...
    for (let i = 0; i < 32; i++) {
        stateRootFromPoolBigInt = (stateRootFromPoolBigInt << 8n) | BigInt(stateRootFromPool[i]);
    }
    
    console.log('    Debug: State root from pool:', stateRootFromPoolBigInt.toString(16).slice(0, 16) + '...');
    console.log('    Debug: State root from merkle:', merkleProofs.stateRoot.toString(16).slice(0, 16) + '...');
    ...
    const scopeBuffer = Buffer.isBuffer(poolState.scope) ? poolState.scope : Buffer.from(poolState.scope);
    ...
    console.log('    Debug: stateRootBigInt type:', typeof stateRootBigInt, 'value:', stateRootBigInt.toString(16).slice(0, 16) + '...');
    ...
    console.log('    Debug: merkleProofs.stateProof is array?', Array.isArray(merkleProofs.stateProof));
    console.log('    Debug: merkleProofs.stateProof length:', merkleProofs.stateProof ? merkleProofs.stateProof.length : 'undefined');
    console.log('    Debug: First 3 stateProof elements:', merkleProofs.stateProof ? merkleProofs.stateProof.slice(0, 3).map(s => typeof s) : 'undefined');
    ...
    // Format: instruction_type (1) + processooor (32) + proof data + public signals
    const withdrawData = Buffer.alloc(553); // 1 + 32 + 256 (proof) + 264 (signals)
    let offset = 0;
    
    withdrawData[offset++] = INSTRUCTIONS.WITHDRAW;
    
    // Add processooor pubkey (32 bytes)
    user.publicKey.toBuffer().copy(withdrawData, offset);
    offset += 32;
    
    // Add proof data (256 bytes total)
