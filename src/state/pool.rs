...
use super::lean_imt::{LeanIMT, new_poseidon_tree};
...
impl PoolStateLeanIMT {
    ...
    pub fn insert_state_commitment(&mut self, commitment: [u8; 32]) -> Result<(), ProgramError> {
        ...
        // Insert into state tree (copy to avoid alignment issues)
        let mut tree = self.state_tree;
        let new_root = tree.insert(commitment)
            .map_err(|_| ProgramError::InvalidArgument)?;
        self.state_tree = tree;
        
        log!("New state tree root after insertion: first byte = {}", new_root[0]);
    ...
    pub fn insert_asp_label(&mut self, label: [u8; 32]) -> Result<(), ProgramError> {
        // Insert into ASP tree (copy to avoid alignment issues)
