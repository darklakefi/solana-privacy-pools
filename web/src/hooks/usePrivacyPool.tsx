...
export interface DepositParams {
  ...
  nonce: number;
  scope: string;
  tokenMint: PublicKey; // Required - the token mint address
}
...
export function usePrivacyPool() {
  ...
  const depositFunds = useCallback(async (params: DepositParams) => {
    ...
    try {
      ...
      const poolStateSeed = getPoolStateSeed(params.tokenMint);
      ...
      // Amount is already in native units (lamports for SOL, smallest unit for SPL tokens)
      const amountNative = params.amount;
  ...
  const withdrawFunds = useCallback(async (params: WithdrawParams) => {
    ...
    try {
      // Amount is already in native units
      const amountNative = params.amount;
  ...
  const getPoolInfo = useCallback(async (tokenMint: PublicKey) => {
    ...
    try {
      ...
      const poolStateSeed = getPoolStateSeed(tokenMint);
      const poolStateAccount = await PublicKey.createWithSeed(
        publicKey,
        poolStateSeed,
        PROGRAM_ID
